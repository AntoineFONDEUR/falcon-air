...
pub const N_INPUT_COLUMNS: usize = 2;
...
const COLUMN_IDS: [&str; N_TRACE_COLUMNS] = ["xor_8_8_a", "xor_8_8_b", "xor_8_8_res"];
    ...
    SimdBackend: BackendForChannel<Blake2sMerkleChannel>,
{
    ...
    // Generate packed columns for two 8-bit limbs and their XOR
    ...
    let x_packed = pack_column(n_rows, |idx| {
        let a = (idx as u32) & 0xFF;
        let b = ((idx as u32) >> 8) & 0xFF;
        M31::from(a ^ b)
    });
